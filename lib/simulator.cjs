...
class MarketSimulator {
  ...
  simulateHourWithCreatives(simulatedHour, dailyBudget, spendSoFarToday, creatives) {
    const { computeKpis } = require("./metrics.cjs");
    const active = creatives.filter((c) => c.status === "active");
    if (active.length < 2) {
      const total = this.simulateHour(simulatedHour, dailyBudget, spendSoFarToday);
      if (!active.length) return { total, breakdown: [] };
      return {
        total,
        breakdown: [{
          creative_id: active[0].id,
          variant: active[0].variant,
          name: active[0].name,
          metrics: total,
          kpis: computeKpis(total),
        }],
      };
    }
    ...
    const rand = this.rand;
    if (rand() < 0.04) this.competitorShock = rand();
    const demand = this.baseMarketVolume * this.hourMultiplier(hour) * this.dayMultiplier(day) * (1 - this.competitorShock * 0.15);
    const totalImpressions = Math.max(0, Math.floor((demand * this.impressionShare(this.currentBid) * (0.85 + rand() * 0.3)) / 24));
    const position = this.auctionPosition(this.currentBid);
    const ctrBase = this.baseCtr * (0.6 + 0.8 * position);
    const effectiveCpc = Math.max(this.marketCpcFloor, this.currentBid * (1 + this.competitionLevel * position) * (0.92 + rand() * 0.16));
    const cr = Math.max(0.02, Math.min(this.baseCr * (0.85 + rand() * 0.3), 0.2));
    const weightSum = active.reduce((a, c) => a + c.traffic_weight, 0);

    const breakdown = [];
    const total = { impressions: 0, clicks: 0, conversions: 0, spend: 0, revenue: 0 };

    for (const creative of active) {
      const w = weightSum > 0 ? creative.traffic_weight / weightSum : 1 / active.length;
      const imp = Math.floor(totalImpressions * w);
      let ctr = ctrBase * creative.ctr_multiplier * (1 - this.creativeFatigue) * (0.9 + rand() * 0.2);
      ctr = Math.max(0.005, Math.min(ctr, 0.12));
      const clicks = imp > 0 ? sampleBinomial(imp, ctr, rand) : 0;
      const spend = clicks * effectiveCpc;
      const conversions = clicks > 0 ? sampleBinomial(clicks, cr, rand) : 0;
      const pm = {
        impressions: imp,
        clicks,
        conversions,
        spend: Math.round(spend * 100) / 100,
        revenue: Math.round(conversions * this.conversionValue * 100) / 100,
      };
      breakdown.push({
